...
{\parindent0pt % disable indentation for formulation
  ...
  \begin{equation}
    ...
    c_{i+1}\ ;\ t \in \Sigma_{c_0}\ ;\ 0 \leq i \leq D - d \}
  \end{equation}
  where, \newline $D$ is the depth of the ontology $O$ \newline $d$ is the depth of the concept $c_0$ in $O$ \newline
  $\subseteq$ denotes the `subclass of' or `is a' relation (taxonomical
  relations of $O$ i.e. $H$); \newline $c_0$ is the directly asserted concept
  of the term $t$ and,\newline $\Sigma_{c_1}$ is the extension
  \cite{hlomani2014ApproachesMethodsMetrics} of $c_0$ (i.e. set of asserted
  individuals) }
\subsection{Ontology Factors}
...
\subsubsection{Structure:}
To investigate the influence of structure, we perform a correlation study
...
We compute Correct Retrieval (CR) of a concept $c$ at depth $d$ in an ontology
$O$ of depth $D$ as the percentage of predictions of the concept at the
correct across all individual terms $t \in T$ associated with that concept.

\begin{equation}
  CR(c) = \frac{\big|\{t \in T; c \in \{y_i\}_{i=1}^{D}; y_d = c\}\big|}{\big|\{t \in T; c \in \{y_i\}_{i=1}^{D}\}\big|}
\end{equation}
