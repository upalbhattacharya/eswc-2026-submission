...
\item The emergent abilities of LLMs grows with \textbf{model size}
  ...
  \cite{kandpal2023LargeLanguageModels}. In such situatResorting to larger
  LLMs can be difficult to fine-tune due to computational resource
  restrictions. As a consequence, larger LLMs are often used `as-is' using
  approaches such as zero-shot, few-shot and Chain-of-Thought prompting. In
  contrast, smaller LLMs offer the ability to accurately fine-tune on specific
  tasks which has been shown to perform better than in-context learning
  strategies \cite{liu2022FewShotParameter}. The choice of an LLM for ontology
