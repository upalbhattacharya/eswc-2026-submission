...
performanve on the the CASE Ontology being significantly lower than the other
...
\ref{tab:entity-label-separation} (G1 and G2). Group 1(G1) disparity exhibits
the lexical similarity between an individual label and its directly-asserted
concepts while Group 2(G2) disparity is the average lexical similarity between
an individual labels and all concept ancestors of the directly-asserted
concept. G1 is highest for the Wines ontology and lowest for the CASE
ontology. LLMs cannot resort to lexical matching to find the directly-asserted
concept for the Wines ontology but might be able to do so for the CASE
ontology. Thus, a great deal of lexical matching can be utilized to identify
the directly-asserted concept across the ontologies. Group 2 disparity
clearly highlights that lexical matching is insufficient for LLMs to infer
concept hierarchies.
Combining insights from the two disparities and the fact that several
individual labels utilize names of the directly-asserted concepts in their
label suggests that LLMs might be able to infer the directly-asserted concepts
from lexical matching but cannot infer hierarchies.
\begin{figure}
    \centering
    ...
    \begin{minipage}{8cm}
    \dirtree{%
    .1 \hlorange{DeviceFacet}.
        .2 \hlorange{MobileAccountFacet}.
            .3 ApplicationAccountFacet.
                .4 \hlorange{BuildFacet}.
                    .5 \hlorange{ProfileFacet}.
                        .6 AddressFacet.
                            .7 \hlorange{ContactFacet}.
                                .8 \textbf{AccountFacet}.
    }
    \end{minipage}
    \caption{Embedding similarity-based Hierarchy. Concepts in \hlorange{orange} are siblings of `AccountFacet'}
    \end{subfigure}
    \begin{subfigure}[t]{0.5\textwidth}
    \begin{minipage}{8cm}
    \dirtree{%
    ...
    \end{minipage}
    \caption{GPT-4o, 1-shot, generic contextualization prediction hierarchy for `AccountFacet' taken from individual correctly identified as instance of `AccountFacet'. Concepts in \hlorange{orange} are siblings of `AccountFacet'. Concepts in \hlgreen{green} are correctly identified ancestors. * are incorrect ancestors but belonging to a hierarchy.}
    \end{subfigure}
    \begin{subfigure}[t]{0.5\textwidth}
    \begin{minipage}{8cm}
    \dirtree{%
    .1 Account.
        .2 \hlorange{DigitalAccountFacet}.
            .3 Observable.
                .4 UcoObject.
                    .5 \hlgreen{UcoThing}.
                        .6 \hlgreen{UcoInherentCharacterizationThing}.
                            .7 \hlgreen{Facet}.
                                .8 \textbf{AccountFacet}.
    }
    \end{minipage}
    \caption{GPT-4o, 5-shot, generic contextualization prediction hierarchy for `AccountFacet' taken from individual correctly identified as instance of `AccountFacet'. Concepts in \hlorange{orange} are siblings of `AccountFacet'. Concepts in \hlgreen{green} are correctly identified ancestors.}
    \end{subfigure}
    \caption{Predicted Hierarchies}
    \label{fig:case-prediction-hierarchy}
\end{figure}
