...
\end{itemize}
The chosen ontologies do not include relational loops and single parentage.
...
investigate correlations between ontology population performance and
structure. In particular, we investigate if deeper or broader ontologies
represent more complex situations for LLMs to perform ontology population.
...
of individuals and concepts. LLM embeddings are indicators of the semantic
`understanding' LLMs develop of words. We employ these embeddings to assess if
base LLM embeddings: 1) associate related concepts of different ontologies
together and, 2) if their representations can associate individuals of an
ontology closely with its concepts.
...
\cite{openai2024EmbeddingModels} model. Despite the embeddings being
primarily representative of the OpenAI models, the observed performance trends
are similar across all tested models (Section \ref{sec:results}), which lead
us to believe that the results can be generalized beyond the GPT
family. Utilizing these embeddings we analyze cluster separation performance of the
two types of entities, 

We supplement the analysis of entity labels by investigating the lexical
matching between concept and individuals. This supplementary analysis aims to
assess whether LLMs resort to simpler string-based 
