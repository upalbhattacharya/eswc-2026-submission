...
smallest ontology: Wines, has the largest number of individuals.  The
Astronomy Ontology has the largest breadth
\cite{gangemi2005OntologyEvaluationValidation} which is an indicator of the
diverseness of its domain. The CASE Ontology is characterized by larger
maximum dispersion \cite{gangemi2005OntologyEvaluationValidation},
highlighting the existence of a large variety of sibling concepts of similar
granularity of the domain. The structural differences between the three
ontologies can help identify the macroscopic structural design implications on
ontology population. Greater depth signifies higher information granularity
that can pose challenges of semantic separation between deeper entities and
their parents. Broader ontologies exhibit greater information diversity that
can confuse LLMs about the relevant domain of entities. Dispersion evaluates
semantic separability as well for closely related sibling entities. We
investigate correlations between LLM-supported ontology population performance
and structure. In particular, we investigate if deeper, broader or dispersed
ontologies (and concepts) represent more complex situations for LLMs to
...
\citet{babaei2025Llms4omMatchingOntologies}, we model the task of ontology
population as a retrieval problem and prompt several LLMs with a modified task
of generating a ranked list of concepts of length up to the depth
...
learning task.
% Modeled as a retrieval task with a retrieval length greater than one provides insight into the LLM's ability to accurately identify and infer concept hierarchies on its own when provided with just the concept labels.
Requiring an LLM to retrieve concepts up to the depth of the ontology provides insight into their ability to infer concept hierarchies when only provided with concept labels.
\chadded[id=UB, comment={\textbf{mAP@D}: Quick reference to the added portion about $\text{mAP@1}$ vs. $\text{mAP@D}$ in Section \ref{sec:results}}]{In addition, as discussed in more detail in Sections \ref{subsec:method-ontology-variability} and \ref{sec:results}, the nature of individual labels across ontologies leads us to believe that evaluating the ability of LLMs on ontology population using only directly-asserted concepts may provide inflated performance indicators due to lexical matching of entity labels.}
...
\subsubsection{Domain Context:}
We experiment with four types of domain
