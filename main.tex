...
\subsubsection{Domain Context:}
We experiment with four types of domain contextualization: 1)
\textbf{Generic}: The LLM is only asked to perform a generic task e.g. ranked
retrieval.  2) \textbf{Ontology}: The LLM is defined as an expert in
ontologies.  3) \textbf{Topic} of an ontology: The LLM is defined as an expert
in the topic of an ontology e.g. \verb|`You are a wine expert'| for the Wines
Ontology \cite{noy2001OntologyDevelopment101} 4) \textbf{Ontology and Topic}:
The LLM is defined as an expert in ontologies and an expert in the
...
However, when analyzing any factor, we draw conclusions with respect to
$\text{mAP@D}$ and do not confine ourselves to a single value for any other
variable to be as general as possible.
...
\subsection{Ontology Population Performance}
