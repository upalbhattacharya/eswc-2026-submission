...
\usepackage{dirtree}
\DTsetlength{0.1em}{0.4em}{0.2em}{0.4pt}{1.6pt}
...
%
\begin{abstract}
  While LLMs have proven their performance in many tasks, it is not yet known
  how different factors, such as prompting strategies, ontology structure and
  entity label semantics influence LLM-driven ontology learning. In this
  paper, we explore the effects of a wide range of ontology-specific and
  LLM-specific factors on the performance of LLM-supported ontology
  population: the ontology learning task of adding new instances to an
  ontology. Using three publicly-available ontologies: Wines, CASE, and
  Astronomy, we study the effects of ontology structure, taxonomy and entity
  labels in conjunction with choice of LLM, prompting approaches, domain
  contexts and response variation on ontology population using LLMs. Our
  findings suggest that LLMs are reasonably capable of performing ontology
  population. Certain structural metrics do not influence their ability to
  perform ontology population. When dealing with entity labels whose LLM
  embedding representations deviate from their ontological nature, LLMs can
  struggle to predict concept hierarchies and may resort to string-based
  matching. However, few-shot prompting is effective at alleviating these
  issues. Our experimentation finds reasoning LLMs perform better than their
  conventional LLM counterparts under zero-shot conditions but few-shot
  prompting with conventional LLMs can achieve similar performance. Larger
  LLMs are consistent in their responses with an increase in temperature
  resulting in a decline in performance. Smaller LLMs have much larger
  variablity and do not exhibit such performance variation with temperature.
  Our study over three ontologies suggests that higher semantic similarity
  between concept and instance labels can be a key factor governing an LLM's
  ability to perform ontology population. Future work will focus on the
  effects of ontology factors defined in the Ontology Learning Layer Cake, on
  LLM factors emphasizing fine-tuning and advanced prompting approaches and,
  on complex inter-factor effects.
  
  \keywords{ Ontology Population \and Ontology Learning \and Ontology
    Engineering \and Large Language Model \and LLM Prompting \and Evaluation}

\end{abstract}
%
\section{Introduction}
\label{sec:introduction}
Manual curation of ontologies is a time and effort-intensive process
...
\cite{he2022BertmapBertBased,chen2023ContextualSemanticEmbeddings,
  he2023LanguageModelAnalysis,giglou2025Llms4omMatchingOntologies} tasks.
...
learning tasks \cite{bombieri2025DoLlmsDream,wang2024DoAdvancedLanguage}.
...
taxonomy and entity labels and the LLM factors: choice of LLM choice,
modelling objective, prompting approach, domain context and response variation
over 3 ontologies using 4 state-of-the-art LLMs.
...
\href{https://anonymous.4open.science/r/llm_ontology_awareness-91C4/}
...
\cite{bombieri2025DoLlmsDream,he2023ExploringLargeLanguage}.For example,
\citet{giglou2023Llms4olLargeLanguage} evaluate LLM performance on term
...
1.0. We investigate response variation using GPT-4o and Llama3-8B on the Wines
and CASE ontologies with a 3-shot prompting approach utilizing an ontology
domain contextualiztion. Statistical consistency is measured across 10
repetitions at each temperature value. Lower temperature values (< 0.5)
highlight performance variation at low response variability while higher
values ($\geq$ 0.5) illustrate performance variation when response variation
is encouraged.
...
improvements in taxonomy retrieval compared to directly assertion.

We observed that $83$ out of the $161$ individuals ($51.5\%$) for the Wines
Ontology, $94$ of the $131$ individuals ($71.7\%$) in the CASE Ontology and,
$17$ out of the $68$ individuals ($25\%$) for the Astronomy Ontology utilize
the name of their directly-asserted concept in their labels. This is
unrealistic in a real-world ontology population scenario. In such
circumstances, lexical matching is sufficient for direct assertion. Thus,
$\text{mAP@1}$ provides an inflated view of the capabilities of LLMs to infer
concept assertions whereas $\text{mAP@D}$ provides a more comprehensive view
of their performance reliability. We also observe higher $\text{map@D}$ scores
compared to $\text{map@1}$ for all experiments on the Wines ontology. 111 of
the 161 individuals in the ontology belong to concepts without descendants,
thus having no hierarchy to predict. Thus, when an LLM is unable to retrieve
the directly asserted concept at the first rank, it is usually able to
retrieve it within the top 4 ranks. While ontology population does not
concern itself directly with taxonomies, analysis using $\text{mAP@D}$
provides a more ontologically grounded evaluation approach. It highlights that
LLMs on their own may not possess good taxonomical sense but in-context
learning with few-shot prompting can largely address this problem.
...
\ref{tab:entity-label-separation}).

\begin{table}
  \centering
  \caption{Entity label metrics for Wines, CASE and Astronomy ontologies
    (Levenshtein shortened to L. for readability)}
  ...
  \begin{tabular}{|l|r|r|r|}
    ...
    \\
    \textbf{Direct L. Distance (G1)}                                         & 10.285                  & 4.137                  & 5.167
    \\
    \textbf{Ancestor L. Distance (G2)}                                       & 12.191                   & 14.092                 & 15.381
    \\
    \hline
  \end{tabular}
\end{table}
...
We hypothesize that, for LLMs to perform ontology population effectively,
cluster separability between the two types of entities should be low (i.e a
lower centroid distance and a higher DBI is better). We observe that the
normalized centroid distance between concept and individual clusters for the
CASE Ontology to be twice as large as the other two ontologies (larger values
indicate greater inter cluster separation). The DBI for the CASE Ontology is
much lower than the other two ontologies. These two entity label metrics
(Table \ref{tab:entity-label-separation}) together suggest that the entity
labels of the CASE Ontology might make ontology population difficult for
LLMs. The performance in Table \ref{tab:results} supports our hypothesis with
performance on the the CASE Ontology being significantly lower than the other
two ontologies in the zero-shot scenario and when using one or two examples
for few-shot prompting. Beyond that, the provision of more examples in
few-shot counters the effects of this entity semantic separation.

% The semantic separation of the entities of the CASE Ontology is visually
% confirmed through a two-dimensional t-SNE \cite{van2008VisualizingData}
% projection of the entities of the three ontologies in Figure
% \ref{fig:entity-semantic-separation}. The individuals of the CASE Ontology are
% well-separated from its concepts in the lower right-hand corner of the
% Figure. No such separation is observable for the entities of the other two
% ontologies.
...
For our supplementary investigation of the extent string similarity can be
leveraged by LLM for ontology population, we compute the compute the
Levenshtein distance \cite{levenshtein1966BinaryCodesCapable} between the
labels of individuals and the two groups outlined previously in Section
\ref{subsec:experiment-ontology-factors}. We report these scores in Table
\ref{tab:entity-label-separation} (G1 and G2). G1 disparity is highest for the
Wines ontology and lowest for the CASE ontology. LLMs cannot resort to lexical
matching to find the directly-asserted concept for the Wines ontology but
might be able to do so for the CASE and Astronomy ontologies. However, much
larger G2 disparity across all ontologies clearly highlights that lexical
matching is insufficient for LLMs to infer concept hierarchies. Combining
insights from the two disparities and the fact that several individual labels
utilize names of the directly-asserted concepts in their label suggests that
LLMs might be able to infer the directly-asserted concepts from lexical
matching but cannot infer hierarchies.

Looking at Table \ref{tab:results}, we observe that under, zero-shot
conditions, LLMs are able to predict the directly-asserted concepts for all
ontologies ($\text{mAP@1}$) but struggle with predicting complete hierarchies
($\text{mAP@D}$), particularly on the CASE ontology. Manually inspecting the
nature of predicted hierarchies in the zero-shot setting, we observed a
large tendency to predict hierarchies based on string similarity.
% Figure\ref{fig:lexical-prediction-hierarchy} showcases the preference for
% string similarity when predicting hierarchies for an individual of the CASE
% ontology.
The influence of string similarity is reduced in the few-shot experiments.

Entity labels greatly influence the ability of LLMs to perform ontology
population. When the semantic separation between the two types of entities is
pronounced, LLMs resort to using string matching to predict hierarchies. The
inclusion of examples during prompting robustly counters the effects of entity
embedding separation and reduces susceptibility to resorting to string
matching for making predictions.
...
%             .3 Facet.
%                 .4 \textbf{AccountFacet}.
%     }
%   \end{minipage}
%   \caption{}
%     % \caption{Ground Truth Hierarchy for `AccountFacet'}
%     \end{subfigure}
%     \begin{subfigure}[b]{0.45\textwidth}
%     \begin{minipage}{\textwidth}
%     \mydirtree{%
%     .1 \hlorange{MobileDeviceFacet}.
%       .2 \hlorange{AccountAuthenticationFacet}.
%          .3 \hlorange{SIMCardFacet}.
%            .4 \hlorange{PhoneAccountFacet}.
%              .5 \hlorange{EmailAccountFacet}.
%                .6 \hlorange{MobileAccountFacet}.
%                   .7 \hlorange{DigitalAccountFacet}.
%                     .8 \textbf{AccountFacet}.
%     }
%   \end{minipage}
%   \caption{}
%     % \caption{GPT-4o, zero-shot, generic contextualization prediction hierarchy for `AccountFacet' taken from individual correctly identified as instance of `AccountFacet'. Concepts in \hlorange{orange} are siblings of `AccountFacet'}
%     \end{subfigure}
%     \caption{a) Ground truth hierarchy of the `AccountFacet' concept from the
%       CASE Ontology b) GPT-4o zero-shot predicted hierarchy for an individual
%       of `AccountFacet'. Concepts highlighted in orange are incorrect
%       hierarchy predictions. }
%     \label{fig:lexical-prediction-hierarchy}
%   \end{figure}
...
% LLMs perform much better on the Wines and Astronomy ontologies.   The $map@1$ scores on the
...
We find that larger (commercial) LLMs outperform their smaller
(open) competitors.
...
that reasoning LLMs do not always outperform general LLMs. For larger models,
we observe that GPT-4o, when provided with a few examples (Table
...
\cite{wang2024DoAdvancedLanguage,nori2024MedpromptO1Exploration,deepseekai2025DeepseekR1Incentivizing}
suggest that reasoning models do not benefit significantly from few-shot
prompting and may even lead to degradation in performance in some situations.
...
Adding examples primarily improves an LLM's ability to infer the correct
...
\begin{figure}
  \begin{subfigure}{0.5\textwidth}
    ...
    \includegraphics[scale=0.25]{assets/wines-ontology-onto-pop-temp-var-exp.pdf}
  \end{subfigure}
  \hspace{0.2cm}
  \begin{subfigure}{0.5\textwidth}
    \centering
    ...
    \includegraphics[scale=0.25]{assets/case-uco-owl-trafficking-onto-pop-temp-var-exp.pdf}
  \end{subfigure}
...
\subsubsection{Response Variation:}
Figure \ref{fig:temp-variation} presents $\text{mAP@D}$ variability exhibited
by GPT-4o and Llama3-8B models run 10 times over Wines and CASE ontologies
under different temperature settings and with all other parameters fixed.

GPT-4o is fairly consistent across all temperature settings on both ontologies
with variability increasing with temperature although it is marginal. Despite
the fact that setting a temperature value of $0.0$ should result in a
deterministic output, both LLMs exhibit response variation at that temperature
although it is quite small for GPT-4o. In contrast, Llama3-8B exhibits a great
deal of variation at each temperature value. There is an observed
degradation in performance with increase in temperature in GPT-4o, with the
highest performance observed at the lowest temperature. This decline is
slightly more pronounced in the CASE Ontology. Similar performance trends are
not observed for Llama3-8B. Performing Welch's T-test for statistically
significant performance difference across both LLMs and ontologies against the
default value of $0.6$, we find performance differences observed by GPT-4o to
be statistically significant for the Wines Ontology at temperatures of 0.0,
0.8 and 1.0 and, for the CASE Ontology at all temperatures. The same test were
found to be statistically insignificant in all scenarios for Llama3-8B except
on the Wines Ontology at temperature 0.0.

% case-uco-owl-trafficking gpt-4o t0.0
% (M = 0.79, SD = 0.01), t(18) = 26.0, p < 0.001
% case-uco-owl-trafficking gpt-4o t0.2
% (M = 0.77, SD = 0.01), t(18) = 16.0, p < 0.001
% case-uco-owl-trafficking gpt-4o t0.4
% (M = 0.78, SD = 0.01), t(18) = 15.7, p < 0.001
% case-uco-owl-trafficking gpt-4o t0.8
% (M = 0.70, SD = 0.01), t(18) = 9.7, p < 0.001
% case-uco-owl-trafficking gpt-4o t1.0
% (M = 0.58, SD = 0.02), t(18) = 31.5, p < 0.001
% wines-ontology llama3-8B t0.0
% (M = 0.70, SD = 0.03), t(18) = 3.9, p < 0.001
% wines-ontology gpt-4o t0.0
% (M = 0.90, SD = 0.00), t(18) = 8.9, p < 0.001
% wines-ontology gpt-4o t0.8
% (M = 0.84, SD = 0.01), t(18) = 15.5, p < 0.001
% wines-ontology gpt-4o t1.0
% (M = 0.79, SD = 0.02), t(18) = 16.4, p < 0.001

For both ontologies, we observe a reduction in average performance with
increase in temperature with GPT-4o. A similar trend is not observed for
experiments with Llama3-8B. 

\section{Conclusion}
\label{sec:conclusion}

We present an analysis of the ontology and LLM factors influencing
LLM-supported ontology population. Through the simple ontology learning task
of ontology population, we investigate the ontology factors: structure,
taxonomy and entity labels and, the LLM factors: choice of LLM, modelling
objective, prompting approach, domain context and response variation over
three domain ontologies and several LLMs. Through extensive experimentation
(288), we find that LLMs are capable of performing ontology population well,
utilizing only their own pre-learnt knowledge and in-context learning.
A detailed analysis of the explored factors highlights the following:
\begin{itemize}
\item Structural complexity of ontologies in terms of size and of depth and
  dispersion of concepts does not seem to affect an LLM's ability to perform
  ontology population.
\item LLMs are capable of identifying directly-asserted concepts but often
 struggle with predicting correct hierarchies. Addition of contextual examples
 overcomes this shortcoming.
\item In some situations, an LLM's semantic understanding of ontology concepts
  and individuals from their labels does not align with their ontological
  nature. In such situations, LLMs struggle to predict ontologically
  meaningful hierarchies and are prone to resorting to simplistic string-based
  matching.
\item Larger LLMs outperform smaller LLMs. Reasoning LLMs can potentially
  perform ontology population better, but conventional LLMs can perform
  equally well in few-shot settings. A smaller reasoning model reported the
  worst overall performance, which requires further investigation.
\item LLMs are capable of identifying direct concept assertions under
  zero-shot settings but struggle with taxonomy prediction. Adding examples
  with few-shot prompting significantly improves performance with the largest
  benefits obtained with three or four examples.
\item When an LLM's lexical understanding of an ontology's entities diverges
  from their ontological nature and string-based similarity cannot be used to
  infer hierarchies, few-shot prompting is an effective tool to mitigate these
  differences and improve performance.
\item Our experimentation with domain contextualization strategies shows
  stronger benefits of the ontology domain contextualization in most
  scenarios. However, the observed differences in performance are fairly
  marginal.
\item Larger LLMs exhibit less response variation over repetitions but smaller
  LLMs show significant variability.
\item In larger LLMs, increase in temperature leads to a decline in ontology
  population performance. Mild variation is observed even under deterministic
  temperature conditions. Significant variation in responses from small LLMs
  renders temperature-based performance variation inconclusive.
\end{itemize}
