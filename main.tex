...
separation using the Davies-Bouldin Index (DBI)
\cite{davies1979ClusterSeparationMeasure} (Table
\ref{tab:entity-label-separation}). We observe that the normalized centroid
distance between concept and individual clusters for the CASE Ontology to be
twice as large as the other two ontologies (larger values indicate greater
inter cluster separation). The DBI metric is an internal clustering evaluation
measure of cluster separability. We hypothesize that, for LLMs to perform
ontology population effectively, the internal cluster separability should be
low (i.e. a higher value is better). The DBI for the CASE Ontology is much
lower than the other two ontologies. Table \ref{tab:entity-label-separation}


 We observe the distance
between the entity centroids for the CASE ontology to be twice that of the
other two ontologies.   is an internal clustering evaluation
